###################################################
# HelloID-Conn-Prov-Target-NokiLock-Students-Delete
# PowerShell V2
###################################################
...
try {
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            $splatRemoveParams = @{
                Uri        = "$($actionContext.Configuration.BaseUrl)/rpc/xml/removeUser"
                Method     = 'POST'
                Headers    = @{'Content-Type' = 'application/x-www-form-urlencoded' }
                Body       = $body
                WebSession = $sessionContext.Session
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Deleting NokiLock-Students account with accountReference: [$($actionContext.References.Account)]"
                $response = Invoke-RestMethod @splatRemoveParams -Verbose:$false
                if ($response.LockerAPI.error.message) {
                    throw [System.Exception]::new("LockerAPI error: $($response.LockerAPI.error.message)")
                }
            }
            else {
                Write-Information "[DryRun] Delete NokiLock-Students account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    $ex = $PSItem
    $auditMessage = "Could not delete NokiLock-Students account. Error: $($_.Exception.Message)"
