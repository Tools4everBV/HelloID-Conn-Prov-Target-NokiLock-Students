###################################################
# HelloID-Conn-Prov-Target-NokiLock-Students-Update
# PowerShell V2
###################################################
...
try {
    ...
    if ($null -ne $correlatedAccount) {
        foreach ($property in $actionContext.Data.PSObject.Properties) {
            if ($null -eq $property.Value) {
                $actionContext.Data.$($property.Name) = ""
            }
        }
    ...
    switch ($action) {
        'UpdateAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Updating NokiLock-Students account with accountReference: [$($actionContext.References.Account)]"
        ...
        'NoChanges' {
            Write-Information "No changes to NokiLock-Students account with accountReference: [$($actionContext.References.Account)]"
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
    ...
    $outputContext.success = $false
    $ex = $PSItem
    $auditMessage = "Could not update NokiLock-Students account. Error: $($_.Exception.Message)"
